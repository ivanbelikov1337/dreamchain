...
const CompletedTab = observer(function CompletedTab({ walletAddress, onOpenProfile }: CompletedTabProps) {
  const { t } = useTranslation()
  const { profileStore } = useStore()

  useEffect(() => {
    profileStore.loadCompletedDreams(walletAddress)
  }, [walletAddress, profileStore])

  const dreams = profileStore.completedDreams
  const isLoading = profileStore.loadingCompleted
