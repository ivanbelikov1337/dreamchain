...
const DonatedTab = observer(function DonatedTab({ walletAddress, onOpenProfile }: DonatedTabProps) {
  const { t } = useTranslation()
  const { profileStore } = useStore()

  useEffect(() => {
    profileStore.loadDonatedDreams(walletAddress)
  }, [walletAddress, profileStore])

  const dreams = profileStore.donatedDreams
  const isLoading = profileStore.loadingDonated
